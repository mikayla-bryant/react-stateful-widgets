...
export default function Programmers() {
  ...
  const [programmerId, programmerList] = useState(listOfAwesome);
  const [activeId, setId] = useState(null);
  //programmerId = programmer, programmerList = setprogrammer, activeId = xid, setId = setxid
  const getNameOfFeatured = () => {
    ...
    return programmerId[activeId-1].name;
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color:  (activeId > 0) ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
